...
void Simulation::set_inheritance(InheritanceParams inher_params)
{
	...
	double delta = inher_params.delta;
	double mother_w,mother_a,mother_b,mother_r;
	double father_w,father_a,father_b,father_r;
	double mother_eA,father_eA,xiA,xiB;

	for(int i=1;i<1+constants::num_gen;++i)
	{
	for(int j=1;j<1+constants::num_gen;++j)
	{
	for(int k=1;k<1+constants::num_gen;++k)
	{
		mother_w=0;mother_a=0;mother_b=0;mother_r=0;
		father_w=0;father_a=0;father_b=0;father_r=0;
		if(i==1){mother_w=1;mother_a=0;mother_b=0;mother_r=0;};
		if(i==2){
			mother_w=(1-xi)*(1-mother_eA)*(1-gamma)*(1-delta)*0.5;
			mother_a=(1-xi)*(1+mother_eA)*0.5;
			mother_b=(1-xi)*(1-mother_eA)*(1-gamma)*delta*0.5;
			mother_r=(1-xi)*(1-mother_eA)*gamma*0.5;
			};
		if(i==3) { mother_w=0; mother_a=0; mother_b=0; mother_r=0; };
		if(i==4) { mother_w=0.5; mother_a=0; mother_b=0; mother_r=0.5; };
		if(i==5) { mother_w=0; mother_a=0; mother_b=0; mother_r=0; };
		if(i==6) { mother_w=0; mother_a=0; mother_b=0; mother_r=0; };
		if(i==7)
			{
			mother_w=(1-xi)*(1-mother_eB)*(1-gamma)*(1-delta)*0.5;
			mother_a=(1-xi)*(1-mother_eB)*(1-gamma)*delta*0.5;
			mother_b=(1-xi)*(1+mother_eB)*0.5;
			mother_r=(1-xi)*(1-mother_eB)*gamma*0.5;};
			};
		if(i==8) { mother_w=0; mother_a=0; mother_b=0; mother_r=0; };
		if(i==9) { mother_w=0; mother_a=0; mother_b=0; mother_r=0; };
		if(i==10) { mother_w=0; mother_a=0; mother_b=0; mother_r=0; };

		if(j==1){father_w=1;father_a=0;father_b=0;father_r=0;};
		if(j==2){
			father_w=(1-father_eA)*(1-gamma)*(1-delta)*0.5;
			father_a=(1+father_eA)*0.5;
			father_b=(1-father_eA)*(1-gamma)*delta*0.5;
			father_r=(1-father_eA)*gamma*0.5;
			};
		if(j==3) { father_w=0; father_a=1; father_b=0; father_r=0; };
		if(j==4) { father_w=0.5; father_a=0; father_b=0; father_r=0.5; };
		if(j==5) { father_w=0; father_a=0; father_b=0; father_r=1; };
		if(j==6) { father_w=0; father_a=0.5; father_b=0; father_r=0.5; };
		if(j==7)
			{
			father_w=(1-father_eB)*(1-gamma)*(1-delta)*0.5;
			father_a=(1-father_eB)*(1-gamma)*delta*0.5;
			father_b=(1+father_eB)*0.5;
			father_r=(1-father_eB)*gamma*0.5;};
			};
		if(j==8) { father_w=0; father_a=0.5; father_b=0.5; father_r=0; };
		if(j==9) { father_w=0; father_a=0; father_b=1; father_r=0; };
		if(j==10) { father_w=0; father_a=0; father_b=0.5; father_r=0.5; };
		if(k==1){inher_fraction[i][j][k]=mother_w*father_w;};
		if(k==2){inher_fraction[i][j][k]=mother_w*father_a+mother_a*father_w;};
		if(k==3){inher_fraction[i][j][k]=mother_a*father_a;};
		if(k==4){inher_fraction[i][j][k]=mother_w*father_r+mother_r*father_w;};
		if(k==5){inher_fraction[i][j][k]=mother_r*father_r;};
		if(k==6){inher_fraction[i][j][k]=mother_r*father_a+mother_a*father_r;};
		if(k==7){inher_fraction[i][j][k]=mother_w*father_b+mother_b*father_w;};
		if(k==8){inher_fraction[i][j][k]=mother_a*father_b+mother_b*father_a;};
		if(k==9){inher_fraction[i][j][k]=mother_b*father_b;};
	};
	};
/*	for(int i=0;i<constants::num_gen;++i)
	{
